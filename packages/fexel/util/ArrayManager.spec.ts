...
import {
	ArrayManager,
	TypedArrayManager,
	TypedArrayVariableManager,
	ArrayVariableManager,
	ArrayBlock,
} from './ArrayManager';
...
describe('Storage', () => {
	it('construct', async () => {
		...
		expect(Array.from(new TypedArrayManager(new Float32Array(2)).data)).to.eql([
			0,
			0,
		]);

		expect(
			Array.from(new TypedArrayVariableManager(new Float32Array(0), 2).data)
		).to.eql([]);
