...
export class ClientWebRTC extends Client {
	...
	constructor(public readonly conf: ClientWebRTCConf) {
		...
		this.peerConnection = new RTCPeerConnection({
			iceServers: conf.iceServers ? conf.iceServers : [
		...
		this.peerConnection.oniceconnectionstatechange = (e) => {
			console.trace('oniceconnectionstatechange', e, this.peerConnection.iceConnectionState);
		...
		this.peerConnection.setRemoteDescription(
			conf.remoteDescription,
			() => {
				this.peerConnection.createAnswer().then(desc => {
					this.peerConnection.setLocalDescription(
						desc,
						() => {
							this.emit('onAnswer', this.peerConnection.localDescription);
						},
						emitError
					);
				})
	...
	sendPayload(payload: Payload): void {
		if (this.peerChannel) {
			this.peerChannel.send(JSON.stringify({
				ts: Date.now(),
				...payload
			}));
		}
