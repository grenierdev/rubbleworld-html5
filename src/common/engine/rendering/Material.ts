...
export type Uniform = UniformInt | UniformFloat | UniformIVec2 | UniformIVec3 | UniformIVec4 | UniformVec2 | UniformVec3 | UniformVec4 | UniformMat2 | UniformMat3 | UniformMat4 | UniformSampler | UniformSampler2D | UniformSamplerCube;
...
export class Material implements IDisposable {
	...
	protected updateUniform(uniform: Uniform) {
		switch (uniform.type) {
			...
			case Type.Mat2:
				if (isArray(uniform.value)) {
					this.gl.uniformMatrix2fv(uniform.location, uniform.transpose === true, uniform.value);
				} else {
					throw new SyntaxError(`Expected uniform ${name} to be an array of number.`);
				}
				break;
			case Type.Mat3:
				if (isArray(uniform.value)) {
					this.gl.uniformMatrix3fv(uniform.location, uniform.transpose === true, uniform.value);
				} else {
					throw new SyntaxError(`Expected uniform ${name} to be an array of number.`);
				}
				break;
			case Type.Mat4:
				if (isArray(uniform.value)) {
					this.gl.uniformMatrix4fv(uniform.location, uniform.transpose === true, uniform.value);
				} else {
					throw new SyntaxError(`Expected uniform ${name} to be an array of number.`);
				}
				break;
